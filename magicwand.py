...
class MagicWand:
    ...
    def preprocess_image(self,image):
        ...
        edge_map = feature.canny(grayimg, sigma=9)
        return edge_map
    ...
    def detect_circles(self,edges):
        ...
        circles = [(cx[i], cy[i], radii[i]) for i in range(len(cx))]
        return circles
    ...
    def calculate_ball_position(self,x,y,r):
        ...
        """
        Z = (self.R * self.focal) / r
        X = (Z) * ((x - self.centerx) / self.focal)
        Y = (Z) * ((y - self.centery) / self.focal)
